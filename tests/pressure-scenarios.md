...
- 连续生成 01、02、03 三份 Analysis 文档。
- 执行 `analysis-human-review`。
- 只在 01-03 全部完成后中断一次等待用户确认。
- 不得在 01 或 02 完成后分别要求用户确认。
...
- 连续生成 05 PRD、06 功能任务、07 UI IA、08 UI Spec、09 原型标注、10 baseline。
- 每个节点执行对应 auto_review。
- auto_review pass 时写入自动检查记录并继续下一节点。
- 不得在 05、06、07、08、09、10 每个文档后请求用户确认。
- 任一 auto_review fail 时进入 repair-run。
