...
使用 `templates/01-analysis-input.md`。它收集产品基本信息、用户现场信息、已有系统材料、市场/竞品材料和待确认问题。

如果用户没有提供足够信息，允许模型补充，但必须标记为“模型假设 / 待确认”，不能伪装成事实。

### 3.2 产品调研与洞察

使用 `templates/02-product-research-insight.md`。它合并市场调研、竞品分析、客户现场调研和用户访谈材料，输出洞察与设计影响。

### 3.3 需求分析

使用 `templates/03-requirement-analysis.md`。它把 `RAW / INS / NEED / PROB` 转成 `REQ`，明确需求边界、排除项、假设和后续设计输入。
