...
- 生成原型输入包。
- 输出 prototype-input。
- 生成 Figma Make prompt。
- 生成 Motiff / Uizard / UX Pilot prompt。
- 基于当前规格做原型验证。
- 输出 Figma MCP 执行 Prompt。
...
1. `prototype-input/` 只能作为原型工具输入包，不得成为产品事实源。
2. 不得新增 `product-spec/` 外没有定义的页面、功能、按钮或业务流程。
3. 样例数据只用于原型，不得回写 PRD、功能任务或 baseline。
4. `SCR / CMP / FEAT / BR / AC` 必须用于 frame、layer、annotation 或 handoff，不得作为用户可见 UI 文案。
