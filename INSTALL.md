...
```text
使用 ssf-product-pm，根据下面的产品想法生成完整产品设计包。要求输出 8 个文档，遵守 templates 中的模板，不要生成传统需求池、优先级矩阵、Roadmap。
...
```text
使用 ssf-product-pm，基于已有功能任务规格生成 UI 信息架构、结构化 UI/交互规格、原型生成 Prompt 与 UI 标注说明。要求所有页面和组件都有 ID，并能追溯到功能、规则和验收标准。
```
