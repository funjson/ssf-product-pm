...
- Analysis 结束后必须人工评审，且只在 01-03 全部完成后统一中断一次；未确认不得默认进入 Design。
- Design 阶段先产品架构，再 PRD、功能任务和 UI。
- 产品架构结束后必须人工评审，未确认不得默认进入后续设计。
- 产品架构之后的 PRD、功能任务、UI、原型标注和 baseline 使用自动自检查；通过后继续，失败时 repair-run。
...
- 原型工具输入包位于实例根目录 `prototype-input/`，不放在 `product-spec/` 内。
